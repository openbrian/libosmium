/*
 ...
 Split OSM files by date from one format into another.
...
struct SplitterHandler : public osmium::handler::Handler {

  SplitterHandler(std::array<osmium::io::Writer*, FILES>& w) : writers(w) {}

  void osm_object(const osmium::OSMObject& obj) {

       int idx = uint32_t(obj.timestamp()) / 86400 - 11000;

       (*writers.at(idx))(obj);
  }

  std::array<osmium::io::Writer*, FILES> writers;

};

void print_help() {
    std::cout << "osmium_splitter [OPTIONS] [INFILE [OUTFILE]]\n\n" \
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        header.set("generator", "osmium_splitter");

        std::array<osmium::io::Writer*, FILES> writers;

        for (int i = 0; i < FILES; i++) {
          std::stringstream filename;
          filename << std::setfill('0') << std::setw(5) << i << output;
          osmium::io::File outfile(filename.str(), output_format);
          osmium::io::Writer* writer = new osmium::io::Writer(outfile, header, osmium::io::overwrite::allow);
          writer->set_buffer_size(2 * 1024 * 1024);
          writers[i] = writer;
        }

        SplitterHandler splitter(writers);

        while (osmium::memory::Buffer buffer = reader.read()) {
            osmium::apply(buffer, splitter);
        }
        for (int i = 0; i < FILES; i++) {
          (*writers[i]).close();
          delete writers[i];
